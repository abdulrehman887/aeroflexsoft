(function(){
  ...
  function productCard(p,mode){const avail=p.variants.filter(v=>v.stock>0),visible=mode==='available'?avail:p.variants;if(mode==='available'&&!avail.length)return '';const first=visible[0]||{image:p.image};return `<article class="product-card"><a href="product.html?id=${encodeURIComponent(p.id)}&mode=${mode}"><div class="product-img"><img src="${first.image||p.image||'assets/aeroflex-logo.png'}" alt="${p.name}">${mode==='all'&&!avail.length?'<span class="badge">ORDER ON REQUEST</span>':''}</div><div class="product-info"><h3>${p.name}</h3><span>${p.category}</span><strong>Rs. ${Number(p.price||0).toLocaleString()}</strong>${mode==='available'?`<small class="ctn-pill">${avail.map(v=>`${v.name}: ${v.stock} CTN`).join(' • ')}</small>`:`<small>${p.variants.map(v=>v.name).filter(Boolean).join(' • ')}</small>`}</div></a></article>`;}
